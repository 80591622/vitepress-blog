...
permalink: /resources/components
categories:
  - 资源库
tags:
  - 资源库
...
2. 在入口组件的 `setup()` 中通过 `provide(teekConfigContext, {})` 添加组件的配置项，`provide` 方式在主题组件的文章里会进行说明

`provide` 方式优先级大于 `defineTeekConfig` 方式。

`defineTeekConfig` 函数内部会进行一些 Vite 插件的初始化，有关 Vite 插件的介绍和如何禁用请看 [Vite 插件](https://vp.teek.top/guide/plugins)。

除此之外，`defineTeekConfig` 函数会注册 Teek 内置的 `Markdown` 拓展，有关 `Markdown` 拓展的介绍请看 [Markdown 拓展](https://vp.teek.top/guide/markdown)。
