---
date: "2022-08-16 10:40:54"
title: git
categories:
  - Git
tags:
  - Git
lastUpdated: "2022-10-20T10:40:54.988Z"
---
...
```bash
# 1. 先把你本地写的新代码 提交！（必须先 commit）
git add .
git commit -m "feat: 本次功能开发"

# 2. 拉取远程自己的分支（无害）
git fetch origin feature-wk

# 3. 拉取最新 sit 代码
git fetch origin sit

# 4. 把你的功能分支 rebase 到最新 sit（无合并提交）
git rebase origin/sit

# 若冲突：解决 → git add . → git rebase --continue

# 5. 推送到你自己的远程分支（必须加 --force-with-lease 安全强推）
git push --force-with-lease origin feature-wk
...
# 8. 把 feature 合并进 sit（无合并提交！）
git merge --ff-only feature-wk

# 9. 推送到远程 sit
git push origin sit

# 10. 回到你的功能分支继续开发（同步最新sit，不再分叉）
