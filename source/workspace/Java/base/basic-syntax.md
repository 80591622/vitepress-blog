...
tags:
  - java
lastUpdated: "2026-09-06T07:13:33.434Z"
---
...
表达式会计算出一个值。整数相除时会舍弃小数部分；只要有一个操作数是浮点数，结果才会保留小数。
...
| 算术     | `+`、`-`、`*`、`/`、`%`              | 求和、相除、取余           |
| 比较     | `>`、`>=`、`<`、`<=`、`==`、`!=`     | 判断条件，结果为 `boolean` |
| 逻辑     | `&&`、<code>&#124;&#124;</code>、`!` | 组合多个布尔条件           |
| 赋值     | `=`、`+=`、`-=`                      | 更新变量的值               |
| 自增自减 | `++`、`--`                           | 计数器加一或减一           |

判断字符串内容不要使用 `==`。`==` 比较引用是否相同，使用 `equals` 比较字符串内容；常量写在前面可以避免变量为 `null` 时抛出异常。
