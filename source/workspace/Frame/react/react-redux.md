---
date: "2024-04-16 13:52:15"
title: Redux 与 React-Redux
categories:
  - Frame
  - react
tags:
  - react
lastUpdated: "2024-04-17T13:52:15.970Z"
---
...
const connect = (mapStateToProps, mapDispatchToProps) => WrappedComponent => {
  class Connect extends Component {
    ...
    setProps() {
      const { store } = this.context;
      let stateProps = mapStateToProps ? mapStateToProps(store.storeState(), this.props) : {}; // 不传为空
  ...
  return ({ dispatch, getState }) =>
    ...
    action => {
      if (typeof action === "function") {
        //判别action的类型，如果action是函数，就调用这个函数
...
1. 接口访问成功，使用redux-saga/effects中的方法put，发起action，这里的put方法和dispatch一样，都是用于发起action；

1. put发起action后，redux的reducers会收到action，从而更新state。
