...
export const useThemeColor = (color: MaybeRef<string>, ignoreList?: string[] | (() => string[] | undefined)) => {
  ...
  const switchLight = () => {
    ...
    (Object.keys(lightVarDefaultMap) as (keyof typeof lightVarDefaultMap)[]).forEach(key => {
    ...
    // 扩展其他 var 变量
    if (isSpread.value) {
      (Object.keys(lightVarSpreadMap) as (keyof typeof lightVarSpreadMap)[]).forEach(key => {
        if (ignoreListConst?.includes(key)) return;
  ...
  const switchDark = () => {
    ...
    (Object.keys(darkVarDefaultMap) as (keyof typeof darkVarDefaultMap)[]).forEach(key => {
      if (ignoreListConst?.includes(key)) return;
    ...
    // 扩展其他 var 变量
    if (isSpread.value) {
      (Object.keys(darkVarSpreadMap) as (keyof typeof darkVarSpreadMap)[]).forEach(key => {
        if (ignoreListConst?.includes(key)) return;
