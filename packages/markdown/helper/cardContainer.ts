...
/** markdown-it 容器中会用到的最小 token 字段（部分插件会对 token 赋值） */
interface FlowToken {
  type: string;
  info: string;
  content: string;
...
export interface CardContainerOption<D extends Record<string, any>, C extends Record<string, any>> {
   ...
   * 渲染 HTML 的回调
   */
  htmlRender: (props: { data: D[]; config: C }, info: string, tokens: FlowToken[], idx: number) => string;
  /**
   * 渲染 HTML 后的回调
   */
  afterHtmlRender?: (props: { data: D[]; config: C }, info: string, tokens: FlowToken[], idx: number) => void;
  /**
